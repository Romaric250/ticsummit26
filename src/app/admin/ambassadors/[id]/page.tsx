...
const EditAmbassadorPage = ({ params }: { params: Promise<{ id: string }> }) => {
  const { id } = use(params)
  ...
  const [currentTicPoints, setCurrentTicPoints] = useState(0)
  const [ticPointsAdjustment, setTicPointsAdjustment] = useState("")
  ...
  const fetchAmbassador = async () => {
    try {
      ...
      if (data.success) {
        ...
        setProfileImage(ambassadorData.profileImage || null)
        const currentPoints = ambassadorData.ticPoints || 0
        setCurrentTicPoints(currentPoints)
        setFormData({
          ...
          contactInfo: ambassadorData.contactInfo || "",
          socialLinks: ambassadorData.socialLinks || {
            linkedin: "",
            twitter: "",
            instagram: "",
            website: ""
          },
          ticPoints: currentPoints,
          spotlightContributions: Array.isArray(ambassadorData.spotlightContributions) 
            ? ambassadorData.spotlightContributions 
            : [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Calculate final TIC points (current + adjustment)
    let finalTicPoints = formData.ticPoints
    if (ticPointsAdjustment !== "") {
      const adjustment = parseInt(ticPointsAdjustment) || 0
      finalTicPoints = currentTicPoints + adjustment
      if (finalTicPoints < 0) finalTicPoints = 0
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        // Update current points and reset adjustment
        if (finalTicPoints !== currentTicPoints) {
          setCurrentTicPoints(finalTicPoints)
          setTicPointsAdjustment("")
          setFormData(prev => ({ ...prev, ticPoints: finalTicPoints }))
        }
        // Optionally stay on page to see the update, or redirect
        // router.push("/admin/ambassadors")
      } else {
        toast.error(`Failed to update ambassador: ${data.error}`)
      }
    } catch (error) {
      console.error("Error updating ambassador:", error)
      toast.error("Failed to update ambassador. Please try again.")
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="min-h-screen bg-gray-50 pt-20">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Edit Ambassador</h1>
                  <p className="text-gray-600 mt-1">Update ambassador profile and information</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <form onSubmit={handleSubmit} className="space-y-8">
            {/* Basic Information - Same as new page */}
            {/* I'll include the same form fields as the new page */}
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{currentTicPoints}</p>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={ticPointsAdjustment}
                    onChange={(e) => {
                      setTicPointsAdjustment(e.target.value)
                      // Also update formData for when they want to set absolute value
                      const adjustment = parseInt(e.target.value) || 0
                      const newValue = currentTicPoints + adjustment
                      setFormData(prev => ({ ...prev, ticPoints: newValue >= 0 ? newValue : 0 }))
                    }}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900"
                  ...
                  />
                  <p className="text-sm text-gray-500 mt-1">
                    Enter positive number to add points, negative to subtract (e.g., +10, -5)
                  </p>
                </div>
                ...
                {ticPointsAdjustment !== "" && (
                  <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                    ...
                    <p className="text-2xl font-bold text-blue-900">
                      {Math.max(0, currentTicPoints + (parseInt(ticPointsAdjustment) || 0))}
                    </p>
                  </div>
                )}

                {/* Alternative: Set Absolute Value */}
                <div className="pt-4 border-t border-gray-200">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setFormData(prev => ({ ...prev, ticPoints: newValue }))
                      setTicPointsAdjustment((newValue - currentTicPoints).toString())
                    }}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900"
                    placeholder="Set absolute value"
                    min="0"
                  />
                  <p className="text-sm text-gray-500 mt-1">
                    Set the total points directly
                  </p>
                </div>
              </div>
            </div>
